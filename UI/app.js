        ...
        $.post(url, {
            ...
            */
            ...
            if (match) {
                ...
                $("#resultHolder").show();
                $("#divClassTable").show();
                let imageHtml = `<img src="${imageData}" class="img-fluid mb-3"/>`;
                let playerHtml = $(`[data-player="${match.class}"]`).html();
                $("#resultHolder").html(imageHtml + playerHtml);
                let classDictionary = match.class_dictionary;
                for(let personName in classDictionary) {
                    let index = classDictionary[personName];
                    let probabilityScore = match.class_probability[index];
                    let cleanName = personName.split("\\").pop();  // extract last part
                    let elementName = "#score_" + cleanName;
